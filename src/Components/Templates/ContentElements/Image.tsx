...
const Image: React.FC<{ data: any }> = props => {
    ...
    const imageCols = Object.keys(props.data.gallery.rows).map((rowKey, indexRow) => {
        return Object.keys(props.data.gallery.rows[rowKey].columns).map((columnKey, indexCol) => {
            ...
            return <Col className={"gallery-item  gallery-item-size-" + props.data.gallery.count.columns} key={rowKey + '-' + columnKey}>
                <a onClick={() => {
    ...
    return <div className="image">
        <ImageLightbox images={images} setShowLightbox={setShowlightbox} showLightbox={showLightbox} photoIndex={photoIndex} setPhotoIndex={setPhotoIndex}/>
